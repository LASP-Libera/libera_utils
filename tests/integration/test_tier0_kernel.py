...
def test_make_static_kernels(curryer_lsk, short_tmp_path, spice_test_data_path):
    ...
    for kernel_config_file in config.get('LIBERA_KERNEL_STATIC_CONFIGS'):
        ...
        generated_kernels.append(
            kernel_maker.make_kernel(kernel_config_file, short_tmp_path, input_data=None)
        )
...
def test_make_spacecraft_kernels(curryer_lsk, noaa20_spacecraft_data, short_tmp_path, spice_test_data_path):
    ...
    with sp.ext.load_kernel([mkrn.mission_kernels, generated_kernels]):
        ...
        exp_data = noaa20_spacecraft_data[
            ['ADGPSPOSX', 'ADGPSPOSY', 'ADGPSPOSZ', 'ADGPSVELX', 'ADGPSVELY', 'ADGPSVELZ']].values
        ...
        exp_data = noaa20_spacecraft_data[['ADCFAQ4', 'ADCFAQ1', 'ADCFAQ2', 'ADCFAQ3', ]].values  # Format: CXYZ
