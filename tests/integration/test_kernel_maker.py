...
def test_make_jpss_spk(mocked_get_current_version_str, test_pds_file_1, short_tmp_path, curryer_lsk):
    ...
    with mock.patch(
        ...
        mock_parsed_args = argparse.Namespace(
            packet_data_filepaths=[str(test_pds_file_1)], outdir=str(short_tmp_path), overwrite=False, verbose=False
        )
...
def test_make_jpss_spk_aws(
    ...
    mock_parsed_args = argparse.Namespace(
        packet_data_filepaths=[str(packet_s3_path)], outdir=str(s3_output_directory), overwrite=False, verbose=False
    )
    kernel_maker.make_jpss_spk(mock_parsed_args)
...
@mock.patch.object(kernel_maker, "datetime", mock.Mock(wraps=datetime))
@mock.patch("libera_utils.kernel_maker.filenaming.get_current_version_str", return_value="v3-14-159")
def test_make_jpss_kernels_from_manifest_no_time_range(
...
@mock.patch.object(kernel_maker, "datetime", mock.Mock(wraps=datetime))
@mock.patch("libera_utils.kernel_maker.filenaming.get_current_version_str", return_value="v3-14-159")
def test_make_jpss_kernels_from_manifest_one_file(
    mocked_get_current_version_str,
...
@mock.patch.object(kernel_maker, "datetime", mock.Mock(wraps=datetime))
@mock.patch("libera_utils.kernel_maker.filenaming.get_current_version_str", return_value="v3-14-159")
def test_make_jpss_kernels_from_manifest_two_files(
    mocked_get_current_version_str,
    setup_kernel_maker_environment_with_manifest,
    add_start_end_time_to_manifest,
    curryer_lsk,
...
@mock.patch.object(kernel_maker, "datetime", mock.Mock(wraps=datetime))
@mock.patch("libera_utils.kernel_maker.filenaming.get_current_version_str", return_value="v3-14-159")
def test_make_jpss_kernels_from_manifest_three_files(
    mocked_get_current_version_str,
    setup_kernel_maker_environment_with_manifest,
    add_start_end_time_to_manifest,
    curryer_lsk,
