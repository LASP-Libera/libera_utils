...
def test_logging_behavior(setup_test_logging, caplog, monkeypatch, tmp_path):
    ...
    libsdp_log = logging.getLogger('libera_sdp')  # top level libera_sdp logger
    assert libsdp_log.level == logging.DEBUG

    libsdp_child_log = logging.getLogger('libera_sdp.child')  # child libera_sdp logger
    assert libsdp_child_log.level == logging.NOTSET

    # Simulates an external library that does NOT inherit from the libera_sdp logger
    ...
    # We want to exclude anything below INFO that doesn't come from a libera_sdp.* logger
...
def test_setup_task_logger(setup_test_logging, tmpdir, monkeypatch):
    ...
    Loggers that inherit from libera_sdp should be capable of logging to debug
    Loggers that don't inherit from libera_sdp should never log debug messages (to exclude external library debug logs)
    ...
    libsdp_log = logging.getLogger('libera_sdp')  #
    assert libsdp_log.level == logging.DEBUG
    ...
    libsdp_child_log = logging.getLogger('libera_sdp.child')
