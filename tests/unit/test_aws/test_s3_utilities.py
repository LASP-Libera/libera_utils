...
def test_s3_utils_put_cli_handler(mock_s3_put_for_processing_step, file_path, algorithm_name, account_suffix):
    ...
    mock_s3_put_for_processing_step.assert_called_once_with(
        expected_file_path, expected_processing_step, account_suffix=expected_suffix
    )
...
def test_s3_put_for_processing_step(tmp_path, make_test_archive_buckets, processing_step, file_name):
...
def test_s3_utils_list_cli_handler(mock_s3_list_files, algorithm_name, account_suffix):
    ...
    # Make the input namespace object
    args = argparse.Namespace(
        func=s3_utilities.s3_list_cli_handler, algorithm_name=algorithm_name, account_suffix=account_suffix
    )
    ...
    expected_processing_step = ProcessingStepIdentifier(algorithm_name)
    expected_suffix = account_suffix
    mock_s3_list_files.assert_called_once_with(expected_processing_step, account_suffix=expected_suffix)
...
def test_s3_list_objects(tmp_path, make_test_archive_buckets, processing_step, file_names):
...
def test_s3_utils_copy_cli_handler(mock_s3_copy_files, from_path, to_path):
    ...
    # Make the input namespace object
    args = argparse.Namespace(
        func=s3_utilities.s3_copy_cli_handler, source_path=from_path, dest_path=to_path, delete=False
    )
