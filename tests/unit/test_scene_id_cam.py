...
from libera_utils.scene_identification.scene_id_algorithm import RUNNER_CONFIGS, collect_input_files


def _collect_fmatch_cam_input_files(manifest):
    """Select the FMATCH-CAM input files from ``manifest`` (the CAM runner's input product)."""
...
def _fake_manifest(filenames):
    ...
    ``collect_input_files`` only accesses ``manifest.files`` and each record's ``filename``, so a
...
class TestCollectFmatchCamInputFiles:
    """Test cases for collecting FMATCH-CAM input files."""
    ...
    def test_selects_only_fmatch_cam_files(self):
        ...
        selected = _collect_fmatch_cam_input_files(manifest)
    ...
    def test_ignores_non_fmatch_cam_libera_products(self):
        ...
        assert _collect_fmatch_cam_input_files(manifest) == []
