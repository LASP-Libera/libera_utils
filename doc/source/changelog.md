...
- FEAT: **FMATCH production pipeline for all five products.** Adds algorithm runners and container `Dockerfile`s for every FMATCH product (`libera_utils/footprint_matching/{cam,cam_camtime,imager,imager_camtime,imager_flash}/`), sharing manifest/dropbox plumbing (including `select_manifest_files_by_product_id` and an `_as_local_path` helper) via an `FmatchRunnerConfig`-parameterized `_runner.py`. Each reads its L1B Daily input from a manifest (`RAD-4CH` for radiometer-timescale, `CAM` for camera-timescale), writes a conformant NetCDF product to the `PROCESSING_PATH` dropbox, and emits an output manifest. Product assembly/writing (`product.assemble_fmatch_dataset` / `product.write_fmatch_product`) now supports all five modes (previously only `CAM_CAMTIME`), assembling radiometer-timescale modes from L1B pass-through arrays. The L1B Daily reads move into `footprint_matching/l1b_inputs.py` (previously buried in the example generator, which is not installed with the package). Only `fmatch_imager.py` takes `--post-year-one`. The FMATCH images add native HDF4/netCDF/GDAL/PROJ because the ancillary readers open third-party science formats directly. `fmatch_imager_camtime.yml` now carries the same camera pixel-block provenance variables as `fmatch_cam_camtime.yml`, tracing each pseudo-footprint back to its source L1B camera pixels.
- FEAT: **FMATCH-IMAGER year-one vs post-year-one variants.** Adds an input-availability variant axis (`FmatchVariant` in `footprint_matching/types.py`) orthogonal to `OperationalMode`; readers may declare a `REQUIRED_VARIANT` and `VariableSpec`s a `required_variant`, and `ReaderRegistry.get_readers_for_mode(...)` / `product.load_fmatch_definition(...)` filter on it. Because the RBSP products (CERES CLDPIX/SSF) are unavailable in year one, the production `fmatch_imager.yml` defaults to `YEAR_ONE` (ERA5-based); the RBSP-based definition is preserved unchanged as `fmatch_imager_post_year_one.yml`. Both encode the same `FMATCH-IMAGER` ProductID. `CLDPIXReader` and `SSFReader` are gated `POST_YEAR_ONE`.
- FEAT: **Ancillary readers and staging.** Extends the ERA5 single-level reader (`era5`) from the two 10 m winds to seven variables (adding 2 m temperature/dewpoint, surface pressure/geopotential, forecast albedo); adds an ERA5 pressure-level reader (`era5_pressure`, five variables × 13 pressure levels flattened to one product variable per variable/level). Repoints `viirs_aod` from the multi-sensor merged granule to the single-sensor `AERDB_D3_VIIRS_NOAA20` Deep Blue granule and adds categorical `aerosol_type` (plus ranked `aerosol_type_{primary,secondary,tertiary}` companions computed during PSF aggregation); the `aod_550` output contract is unchanged. The new ERA5 fields are gated to year-one FMATCH-IMAGER. Adds `FMATCH_ANCILLARY_PATH` and `footprint_matching/ancillary.py`, which resolves the staged granule tree (one subdirectory per reader registry key) from the readers required per mode/variant rather than hard-coding them — currently non-strict, since nothing consumes the files yet.
- FEAT: **SCENE-ID imager runners.** Adds the SCENE-ID-IMAGER and SCENE-ID-IMAGER-FLASH runners and product definitions (`scene_id_imager{,_flash}.yml`), reusing the existing `SceneIdRunnerConfig`-parameterized `_runner.py`. Unlike SCENE-ID-CAM, the imager products also run the **TRMM** classification (carrying `surface_wind`, `cloud_phase`, `optical_depth`, `scene_id_trmm`, and the `scene_bin_trmm_*` bounds). SCENE-ID-IMAGER reads the post-year-one FMATCH-IMAGER and raises a clear error if handed a year-one file (which has no cloud-phase source); FLASH fills `cloud_phase` with NaN, limiting its TRMM matches to phase-unbounded scenes. Adds `aux_scene_id_imager` / `aux_scene_id_imager_flash` identifiers.
- MAINT: The shared FMATCH reader `FootprintData._extract_data_from_fmatch` is now driven by a declarative per-product column map (`_FmatchColumn`: source name, dtype, optional scale/transform) instead of a hard-coded CAM variable set, so the CAM, IMAGER-FLASH, and post-year-one IMAGER readers share one extraction path and reuse the existing derived-variable calculators. Adds a placeholder `CLDPIXCloudPhase` enum + `map_cldpix_phase_to_trmm` (`TODO[LIBSDC-817]`: the CLDPIX code meanings are unconfirmed).
- MAINT: `SceneDefinition._identify_vectorized` now sizes the scene-ID accumulator to the definition's largest scene id (`np.min_scalar_type`) instead of a fixed `uint8`. ERBE/unfiltering keep `uint8`, but TRMM assigns IDs up to 650, which would silently wrap in a byte; `scene_id_trmm` is declared `uint16` accordingly.
- FIX: The Libera WFOV camera cloud fraction is now scaled to percent on ingest into scene identification. `cloud_fraction_camera` is a `[0, 1]` fraction in the FMATCH-CAM products, but the scene bins and SCENE-ID `cloud_fraction` variable are percent `[0, 100]`; the previous unscaled pass-through placed essentially every footprint in the clearest bin. It is now multiplied by 100, changing SCENE-ID-CAM(-CAMTIME) `cloud_fraction` values and the ERBE/unfiltering scene IDs for any non-clear footprint.
- FIX: `CLDPIXReader` mis-geolocated every pixel. The CERES CLDPIX variable named `Latitude` holds **colatitude** (`valid_range [0, 180]`, 0° at the North Pole), but the reader treated it as a latitude and filtered it to −90…90 — placing northern pixels at the wrong latitude and silently discarding the entire southern hemisphere. It now range-checks against 0…180 and converts with `latitude = 90 − colatitude`.
- FIX: Fill values were never removed from swath-reader fields that declare no `valid_range`. `apply_fill_and_valid_range` compared against the decimal literal `3.4028235e38`, which does not equal the float32 fill stored in the CERES files once widened to float64, so CLDPIX `cloud_top_height` carried raw ~3.4e38 fill into the tile. The fill test now also compares against the sentinel's float32 round-trip.
- DOCS/TEST: Replaces `scripts/generate_fmatch_example_products.py` and `scripts/verify_readers.py` with notebooks (`notebooks/generate_example_products.ipynb`, `notebooks/verify_readers.ipynb`); the generator notebook additionally chains the FMATCH-CAM(-CAMTIME) examples through the real SCENE-ID runners to also produce example SCENE-ID-CAM(-CAMTIME) files, and the verifier documents/visualizes every ancillary reader against real granules. Adds `tests/integration/test_fmatch_runner.py` (full manifest-in/manifest-out workflows) plus unit tests for ancillary resolution, the L1B readers, and per-mode assembly, backed by new `make_l1b_radiometer_fixture` / `make_l1b_camera_fixture` fixtures.
