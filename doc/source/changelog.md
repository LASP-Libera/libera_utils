...
- FEAT: Add `libera_utils.l1a.data_time_extractors` for per-file science data-time spans without full L1A assembly, covering every `DATA_TIME_INDEXED_APIDS` member (WFOV SOP FSW image times; RAD/CAL/AXIS sample epoch+period or per-sample times). `extract_data_time_range` returns `None` for a WFOV file with no `SOP` packet in it, which is expected when a large image's mem-dump is chunked across files or downlink passes.
- FEAT: `jpss_sc_pos` (APID 11) is data-time indexed. Its span covers both of the packet's sample groups — an ephemeris sample time (`ADGPS`) and an attitude sample time (`ADCFA`), which the spacecraft applies independently — running from the earliest time either group reports to the latest. A span records the extent of data present in a file, not the range over which all of its timeseries are simultaneously available; a consumer needing ephemeris and attitude together (geolocation does) intersects the two sample-time ranges itself.
...
- FEAT: Manual ingest (`s3-utils put` / `manual_ingest_data_products`) accepts demuxed ground CCSDS files. `verify_ingestion` checks archival and the base File Metadata row for these; the searchable per-APID row is not verified.
- FEAT: Extracted time spans drop timestamps outside `MIN_VALID_TELEMETRY_TIME`–`MAX_VALID_TELEMETRY_TIME` (defaults `2020-01-01` and `2045-01-01`, both config-overridable), along with `NaT`. Ground captures can carry a packet whose onboard clock has not yet been time-synced, decoding to just after `CCSDS_EPOCH` (1958-01-01), which would otherwise stretch a span across ~68 years.
