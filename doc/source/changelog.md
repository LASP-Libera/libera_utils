...
- BREAKING: Replace calibration event `DataProductIdentifier` values `*-COMBINED` with ObsID-specific products (`GAIN`, `NOISE`, `SWC-{λ}NM`, `LWC-{310,320,330,300,305}K`, `SOLAR-{ch}-{PRI,SEC,TER}`) and add matching L1A trimmed NOM-HK products for Step-1 preprocessor outputs.
- BREAKING: L1A trimmed NOM-HK products are now one per _calibration dependency family_ rather than one per ObsID: the 40 `NOM-HK-<EVENT>-TRIMMED` members are replaced by 10 `NOM-HK-<FAMILY>-FAMILY-TRIMMED` members (`GAIN`, `SWC`, `LWC`, `SOLAR`, `LUNAR`, `RAD-VIIRS-LUNAR`, `CT-VIDEO`, `RAPS-VIDEO`, `DARKS`, `WFOV-VIIRS-LUNAR`). ObsIDs that a downstream algorithm processes identically share one TRIMMED product, so libera*cdk deploys one processing step per input dependency instead of one per ObsID. Each ObsID keeps its own CAL product, and the ObsID itself remains readable from the `ICIE\_\_SW_OBSID*\*` variable inside each trimmed file.
- BREAKING: Replace the 25 ObsID-specific radiometer `ProcessingStepIdentifier` members with four family steps (`cal-gain-family`, `cal-swc-family`, `cal-lwc-family`, `cal-solar-family`); each declares every CAL product its family's ObsIDs can yield. Lunar and camera family steps remain deferred (`TODO[LIBSDC-811]`).
...
- FEAT: Add radiometer lunar ObsIDs 448/449 (`LUNAR-SOUTH-POLE` / `LUNAR-NORTH-POLE`) and RAD/WFOV ObsIDs 513/514 (`{RAD,WFOV}-VIIRS-LUNAR-{POS,NEG}-START`, distinct ProductIDs per source and a separate TRIMMED family per source); cal-combine / PSIs deferred.
- FEAT: Add `libera_utils.l1a.nom_hk_trim` helpers to detect contiguous ObsID runs in decoded NOM-HK and write one trimmed file per run, stamped with that run's family ProductID (warns if the same ObsID recurs in a day; different ObsIDs of one family do not warn). Several files per day normally share a family ProductID and are distinguished by their filename time ranges. Intended for the L1A preprocessor (CDK wiring follow-on).
- FEAT: Add `libera_utils.obsids.TRIM_FAMILIES` and `get_family_specs()` so a downstream cal step enumerates the ObsIDs — and CAL products — of its trimmed input family without hand-maintaining the mapping.
- FEAT: Registry import-time validation now rejects a CAL product claimed by more than one ObsID, and a TRIMMED family registered on both the RAD and WFOV ObsID fields. The previous "TRIMMED product claimed by more than one ObsID" check is removed, since sharing is what defines a family.
