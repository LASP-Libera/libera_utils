...
- FEAT: Add `libera_utils.l1a.ground_ccsds.scan_ground_ccsds_file`, returning one file's packet and science data time spans for File Metadata ingest. It raises `GroundCcsdsScanError` when no packet-time span is possible, and returns the packet span with `degraded_reason` set when only the science data times are unavailable.
