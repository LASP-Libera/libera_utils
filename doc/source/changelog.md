...
- FEAT: **Footprint matching (FMATCH) ancillary readers.** Adds the `libera_utils.footprint_matching.readers` plugin subsystem — a `GriddedDataReader` template base with auto-registration, a `ReaderRegistry`, shared tile value-types, and swath rasterization / lazy HDF I/O helpers. Nine readers turn one external ancillary dataset each into a uniform gridded tile: ERA5 single-level (7 variables) and 37-level pressure fields, VIIRS Deep Blue AOD (`AERDB_D3_VIIRS_NOAA20`) with a categorical `aerosol_type`, VIIRS cloud and BRDF, CERES SSF/CLDPIX, IGBP land cover, and NSIDC NISE sea ice. The heavy geospatial / HDF stack (`pyproj`, `pyhdf`, `h5py`) is isolated behind the optional `fmatch` extra and imported lazily, so the package imports in a core-only install.
- FEAT: **FMATCH and SCENE-ID product definitions.** Adds product definitions and `DataProductIdentifier`s for the FMATCH-CAM(-CAMTIME), FMATCH-IMAGER(-CAMTIME/-FLASH), and SCENE-ID-IMAGER(-FLASH) products. FMATCH-IMAGER additionally carries the RBSP CLDPIX/SSF cloud fields and the extended SSF cloud/aerosol/albedo variables (multi-axis SSF flattened to 1-D per footprint, gated to IMAGER via `VariableSpec.only_modes`). `cloud_fraction_camera` in FMATCH-CAM(-CAMTIME) is defined in percent `[0, 100]` to match the CF-CAM input.
...
- DOCS/TEST: Adds unit tests and synthetic-input fixtures for every reader, and documents the `fmatch` extra and the `libhdf4` build requirement for `pyhdf`.
...
- FEAT: SCENE-ID-CAM-CAMTIME stores one image subsection per record on a `PSEUDOFOOTPRINT` axis and passes the FMATCH footprint identifiers (camera time, pixel-index ranges, PSF bbox, boresight geolocation) straight through. `CAMERA_TIME` is a non-unique coordinate on `PSEUDOFOOTPRINT` (an image is segmented into many, possibly overlapping, subsections that share its time); the pixel-block extent is carried as the four inclusive `camera_pixel_{x,y}_{min,max}` int32 coordinates on the `(CAMERA_TIME, PSEUDOFOOTPRINT)` grid.
