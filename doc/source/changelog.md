...
- FEAT: **Footprint matching (FMATCH) subsystem.** Adds the `libera_utils.footprint_matching` package — camera pseudo-footprint geometry/PSF/segmentation, ancillary-input readers, and production runners with Dockerfiles for all five FMATCH products (`cam`, `cam_camtime`, `imager`, `imager_camtime`, `imager_flash`). Each runner reads its L1B Daily input and staged ancillary granules from a manifest, writes a conformant NetCDF product to the dropbox, and emits an output manifest; product definitions and `DataProductIdentifier`s are added for every product.
- FEAT: **Ancillary readers.** New `readers/` package for the external inputs — ERA5 single-level (7 variables) and 37-level pressure fields, VIIRS Deep Blue AOD (`AERDB_D3_VIIRS_NOAA20`) with a categorical `aerosol_type`, CERES SSF/CLDPIX, IGBP, NSIDC, and BRDF — driven by a declarative per-product column map and resolved from a staged granule tree via `footprint_matching/ancillary.py` (`FMATCH_ANCILLARY_PATH`). FMATCH-IMAGER additionally carries the RBSP CLDPIX/SSF cloud fields and the extended SSF cloud/aerosol/albedo variables (multi-axis SSF flattened to 1-D per footprint, gated to IMAGER via `VariableSpec.only_modes`).
- FEAT: **SCENE-ID imager runners.** Adds the SCENE-ID-IMAGER and SCENE-ID-IMAGER-FLASH runners, product definitions, and identifiers; these read the FMATCH-IMAGER products and, unlike SCENE-ID-CAM, also run the TRMM classification.
- FIX: **Camera cloud fraction is carried in percent end-to-end.** `cloud_fraction_camera` in FMATCH-CAM(-CAMTIME) is now `units: percent` to match the CF-CAM inputs, and scene identification ingests it unchanged (the previous `×100` rescale is removed).
- FIX: `CLDPIXReader` mis-geolocated every pixel — the CERES CLDPIX `Latitude` variable holds **colatitude** (0° at the North Pole), so the reader discarded the entire southern hemisphere. It now converts `latitude = 90 − colatitude`.
- FIX: Fill values were never removed from swath-reader fields declaring no `valid_range` (the float32 fill widened to float64 did not equal the sentinel), leaving raw ~3.4e38 fill in fields such as CLDPIX `cloud_top_height`.
- MAINT: `scene_id_trmm` is now `uint16` and the scene-ID accumulator is sized to the definition's largest scene id (TRMM assigns IDs up to 650, which wrapped in the prior `uint8`).
- DOCS/TEST: Adds the FMATCH geometry, camera-segmentation, example-generation, and reader-verification notebooks (replacing the old generation/verification scripts) and unit + integration tests across the new subsystem.
