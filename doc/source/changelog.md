...
- BREAKING: The `PACKET` axis of an L1A product is in acquisition order (files placed in time, byte order within a file) and is no longer sorted by packet time. Published `{group}_packet_index`, `CAMERA_PACKET_INDEX` and `PACKET_IMAGE_ID` values change. Instrument packet time steps backward on 0.2-1.8% of packets while `SRC_SEQ_CTR` advances by one (LIBSDC-830), so the old sort permuted packets away from the order they were taken in. On a real APID-1040 granule that cost more than half the images: WFOV stitching requires each image's packets to occupy a contiguous run of PACKET rows, and the sort produced 321 images with 103,355 packets discarded where acquisition order produces 669 images with 2,023 discarded. Consumers that assume a time-ordered packet axis must sort locally; `numpy.searchsorted` on packet times is silently wrong without it.
- FEAT: Add `libera_utils.l1a.packet_ordering`: `order_packet_files` places multi-file input in time by the median packet time of each file's first 64 packets (not the filename, which for `L0Filename` encodes an EDOS creation time, and not the first `SSC`, which carries no cross-file ordering information); `check_packet_acquisition_order` cross-checks the axis against `SRC_SEQ_CTR` and reports one summary per granule; `unwrap_sequence_counter` and `find_segment_breaks` handle rollover, gaps and resets. A missing `SRC_SEQ_CTR` warns and trusts input order.
...
- FEAT: Add `libera_utils.l1a.quality`: a `GranuleQualityRecord` of per-granule counters, a `NOMINAL`/`DEGRADED`/`SUSPECT` rollup with tunable `QualityThresholds`, and a `.qa.json` sidecar writer. `parse_packets_to_l1a_dataset` takes an optional `quality_record=` to fill in, and writes the counters as global attributes on every L1A product, zeros included. All twelve L1A product definitions declare the vocabulary, `LiberaDataProductFilename.qa_report_filename` derives the sidecar path, and UMM-G `extract_additional_attributes` and `QAStats` carry the numbers to CMR.
- BREAKING: A duplicate timestamp whose rows carry different data is dropped and counted rather than raising, in both ground and flight mode; `_drop_duplicates` and `_validate_duplicate_values` take `strict=True` to restore raising, and return a `DuplicateReport` instead of a bare count. Identity, not the ground/flight distinction, is what makes a duplicate safe to drop, and raising does not recover the discarded measurement while a recorded count leaves the loss visible. `DuplicateValueMismatchCount` on the product is the number that matters: in DITL2 it is ~10,200 per affected RAD granule, and zero on an unaffected one.
...
- FEAT: `jpss_sc_pos` (APID 11) is data-time indexed. Its span runs from the earliest to the latest time reported by either of the packet's independently timestamped sample groups (`ADGPS` ephemeris, `ADCFA` attitude), so it records the extent of data present rather than the range both groups cover. A consumer needing ephemeris and attitude together must intersect the two sample-time ranges itself.
- FEAT: Add demuxed ground-test CCSDS support: `LiberaGroundCcsdsFilename` and `DataProductIdentifier.l0_ground_ccsds` for the `LIBERA_SDC_<apid>_ccsds_<yyyy>_<doy>_<hh>_<mm>_<ss>` naming (L0 archive prefix `GroundCCSDS/<apid>/<yyyy>/<mm>/<dd>/`). Each file holds one APID, read from the basename, with no record header before the CCSDS primary header. The name's time fields are the bin start the file was cut from, not the span of the packets inside it.
- FEAT: Add `libera_utils.l1a.ground_ccsds.scan_ground_ccsds_file`, returning one file's packet and science data time spans for File Metadata ingest. It returns `None`, logging the reason, when no packet-time span is possible at all, and returns the packet span with `degraded_reason` set when only the science data times are unavailable.
- FEAT: Manual ingest (`s3-utils put` / `manual_ingest_data_products`) accepts demuxed ground CCSDS files. `verify_ingestion` checks archival and the base File Metadata row for these; the searchable row is not verified.
- FIX: `verify_ingestion` treats its File Metadata record count as a floor rather than an exact match. A file's data can span more than one day, giving it a searchable record per applicable date, so an L0 PDS with a multi-day span (as a `jpss_sc_pos` file can have) no longer polls to timeout on a count of 3 against an expected 2.
- FEAT: Extracted time spans drop `NaT` and timestamps outside `MIN_VALID_TELEMETRY_TIME`-`MAX_VALID_TELEMETRY_TIME` (defaults `2020-01-01` and `2045-01-01`, both config-overridable). A ground capture can carry a packet whose onboard clock has not yet been time-synced, decoding to just after `CCSDS_EPOCH` (1958-01-01), which would otherwise stretch a span across ~68 years.
- FEAT: `extract_data_time_range` and `parse_packets_to_l1a_dataset` accept an optional `skip_header_bytes=` override, falling back to `SKIP_PACKET_HEADER_BYTES` when omitted, so one process can handle both flight and ground files without mutating shared config.
- FEAT: `libera_utils.l1a.packets.expand_sample_times` is now public (was `_expand_sample_times`), and `parse_packets_to_l1a_dataset` takes `ground_data` and `verbose` as keyword-only arguments.
