...
- BREAKING: **FMATCH-CAM-CAMTIME and FMATCH-IMAGER-CAMTIME move to a `FOOTPRINT` record axis** (matching SCENE-ID-CAM-CAMTIME): one pseudo-footprint per record, with `CAMERA_TIME` a non-unique coordinate. The camera pixel-block extent becomes the 2-D `camera_pixel_x` / `camera_pixel_y` inclusive `(min, max)` range coordinates, replacing the four `camera_pixel_{x,y}_{start,stop}` variables; boresight `center_pixel_{x,y}` are retained as FMATCH-only (not carried to SCENE-ID).
- FEAT: **FMATCH production pipeline for all five products.** Adds runners and Dockerfiles for every FMATCH product (`cam`, `cam_camtime`, `imager`, `imager_camtime`, `imager_flash`), sharing manifest/dropbox plumbing via an `FmatchRunnerConfig`-parameterized `_runner.py`. Each reads its L1B Daily input from a manifest, writes a conformant NetCDF product to the dropbox, and emits an output manifest; product assembly now supports all five modes (previously only `CAM_CAMTIME`). L1B reads move into `footprint_matching/l1b_inputs.py`; only `fmatch_imager` takes `--post-year-one`; the FMATCH images add native HDF4/netCDF/GDAL/PROJ for the ancillary readers.
- FEAT: **FMATCH-IMAGER year-one vs post-year-one variants.** Adds a `FmatchVariant` input-availability axis orthogonal to `OperationalMode`; readers and `VariableSpec`s may require a variant, and the reader registry / definition loader filter on it. Because the RBSP products (CERES CLDPIX/SSF) are unavailable in year one, `fmatch_imager.yml` defaults to `YEAR_ONE` (ERA5-based) while the RBSP-based definition is preserved as `fmatch_imager_post_year_one.yml`; both encode the same `FMATCH-IMAGER` ProductID.
- FEAT: **Ancillary readers and staging.** Extends the ERA5 single-level reader from 2 to 7 variables, adds an ERA5 pressure-level reader (5 variables × 13 levels), and repoints `viirs_aod` to the single-sensor `AERDB_D3_VIIRS_NOAA20` Deep Blue granule with a new categorical `aerosol_type` (`aod_550` unchanged). Adds `FMATCH_ANCILLARY_PATH` and `footprint_matching/ancillary.py`, which resolves the staged granule tree from the readers required per mode/variant (currently non-strict).
- FEAT: **SCENE-ID imager runners.** Adds the SCENE-ID-IMAGER and SCENE-ID-IMAGER-FLASH runners and product definitions, reusing the `SceneIdRunnerConfig`-parameterized `_runner.py`, plus `aux_scene_id_imager{,_flash}` identifiers. Unlike SCENE-ID-CAM, the imager products also run the TRMM classification. SCENE-ID-IMAGER reads the post-year-one FMATCH-IMAGER and errors on a year-one file (no cloud-phase source); FLASH fills `cloud_phase` with NaN.
- FEAT: `FootprintData.from_fmatch_cam_camtime` reads the operational FMATCH-CAM-CAMTIME product onto the `FOOTPRINT` axis, passing `CAMERA_TIME` and the 2-D pixel-range coordinates through to SCENE-ID-CAM-CAMTIME. Removes the one-off demo generator in favor of this reader.
- FIX: The Libera WFOV camera cloud fraction is now scaled to percent (×100) on ingest into scene identification. The previous unscaled `[0, 1]` pass-through against percent `[0, 100]` bins placed nearly every footprint in the clearest bin; this changes SCENE-ID-CAM(-CAMTIME) `cloud_fraction` values and ERBE/unfiltering scene IDs for non-clear footprints.
- FIX: `CLDPIXReader` mis-geolocated every pixel — the CERES CLDPIX `Latitude` variable actually holds **colatitude** (0° at the North Pole), so the reader treated it as latitude and discarded the entire southern hemisphere. It now range-checks 0…180 and converts `latitude = 90 − colatitude`.
- FIX: Fill values were never removed from swath-reader fields declaring no `valid_range`; the sentinel literal didn't equal the float32 fill widened to float64, so CLDPIX `cloud_top_height` carried raw ~3.4e38 fill. The fill test now also compares against the sentinel's float32 round-trip.
- MAINT: The shared FMATCH reader is now driven by a declarative per-product column map (source name, dtype, optional scale/transform) instead of a hard-coded CAM variable set, so the CAM, IMAGER-FLASH, and post-year-one IMAGER readers share one extraction path. Adds a placeholder `CLDPIXCloudPhase` enum and `map_cldpix_phase_to_trmm` (`TODO[LIBSDC-817]`).
- MAINT: `SceneDefinition._identify_vectorized` sizes the scene-ID accumulator to the definition's largest scene id instead of a fixed `uint8` (TRMM assigns IDs up to 650, which would wrap in a byte); `scene_id_trmm` is declared `uint16`.
- DOCS/TEST: Replaces `scripts/generate_fmatch_example_products.py` and `scripts/verify_readers.py` with notebooks; the generator notebook also chains the FMATCH-CAM(-CAMTIME) examples through the real SCENE-ID runners. Adds `tests/integration/test_fmatch_runner.py` plus unit tests for ancillary resolution, the L1B readers, and per-mode assembly.
