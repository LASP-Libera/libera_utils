...
- DOCS/TEST: Adds `notebooks/verify_readers.ipynb` (replacing the old generation/verification scripts), unit tests and synthetic-input fixtures for every reader, and documents the `fmatch` extra and the `libhdf4` build requirement for `pyhdf`.
