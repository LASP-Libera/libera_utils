...
performed by :meth:`FootprintData.from_fmatch_cam_camtime` and declared in ``scene_id_cam_camtime.yml``.
"""
...
# Scene classifications produced by the SCENE-ID-CAM-CAMTIME product.
SCENE_ID_CAM_CAMTIME_SCENE_TYPES = ["erbe", "unfiltering"]

# Path to the SCENE-ID-CAM-CAMTIME product definition.
PRODUCT_DEFINITION_PATH = (
    Path(__import__("libera_utils").__file__).parent / "data" / "product_definitions" / "scene_id_cam_camtime.yml"
)

# All the parameters that make this the camera-timescale CAM runner (see SceneIdRunnerConfig).
