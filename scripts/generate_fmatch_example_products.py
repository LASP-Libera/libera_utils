...
Run it with::
    ...
    # Anchor the synthetic products (and their filenames) to a chosen UTC date:
    python scripts/generate_fmatch_example_products.py --date 2028-02-12
    ...
    # Chain the two operational stages: also run SCENE-ID-CAM on the generated
    # FMATCH-CAM file so both products land in --output-dir on the SAME date
    # (SCENE-ID-CAM inherits its RADIOMETER_TIME axis from the FMATCH-CAM input):
    python scripts/generate_fmatch_example_products.py --date 2028-02-12 --run-scene-id
...
def _synthesize_time_coordinate(n: int, base_time: np.datetime64 = BASE_TIME) -> np.ndarray:
    ...
    base_time : np.datetime64
        Anchor for the first footprint's timestamp. Defaults to :data:`BASE_TIME`;
        overridden by ``--date`` so the caller can stamp the example (and any
        product derived from it, e.g. SCENE-ID-CAM) with a chosen date.
...
def build_synthetic_data(
    ...
    base_time : np.datetime64
        Anchor for the synthetic time coordinate. Ignored on the L1B pass-through
        path (which carries real timestamps); used for every synthetic mode.
    ...
    if use_l1b:
        ...
        data[time_variable] = _synthesize_time_coordinate(n, base_time)
...
def generate_example_product(
    ...
    base_time : np.datetime64
        Anchor for the synthetic time coordinate (see :func:`build_synthetic_data`).
    ...
    """
    definition = load_fmatch_definition(mode)
    data, time_variable = build_synthetic_data(mode, n_footprints, l1b_passthrough, base_time)
...
def run_scene_id_cam(fmatch_cam_path: Path, output_dir: Path) -> Path:
    """Run the SCENE-ID-CAM algorithm on a generated FMATCH-CAM file.

    Chains the two operational stages so a single command yields both products on
    the *same date*: the SCENE-ID-CAM runner is written on the same
    ``RADIOMETER_TIME`` axis it reads from its FMATCH-CAM input, so whatever date
    the example FMATCH-CAM file carries (from ``--date`` or ``--l1b-file``) is
    inherited verbatim by the SCENE-ID-CAM product.

    The runner is manifest-driven and reads its output directory from the
    ``PROCESSING_PATH`` environment variable, so this helper builds an in-memory
    input :class:`~libera_utils.io.manifest.Manifest` listing the FMATCH-CAM file,
    writes it into ``output_dir``, points ``PROCESSING_PATH`` there, and invokes
    the runner. Everything stays local - no AWS interaction.

    Parameters
    ----------
    fmatch_cam_path : Path
        Path to the FMATCH-CAM NetCDF file produced by
        :func:`generate_example_product` for ``OperationalMode.CAM``.
    output_dir : Path
        Directory used as the processing dropbox (holds the input/output manifests
        and the written SCENE-ID-CAM product).

    Returns
    -------
    Path
        Path to the written SCENE-ID-CAM NetCDF product file.
    """
    # Imported lazily so the default (FMATCH-only) generation path does not pull in
    # the scene-identification stack.
    from libera_utils.constants import ManifestType
    from libera_utils.io.manifest import Manifest
    from libera_utils.scene_identification import scene_id_cam

    # Resolve the dropbox to an absolute path. The runner writes its product into
    # PROCESSING_PATH and then registers it on the output manifest, and manifest
    # entries must be absolute (Manifest.check_file_structure rejects relative
    # paths), so a relative --output-dir (e.g. "example_outputs/") would otherwise
    # fail once the product is added to the output manifest.
    output_dir = output_dir.resolve()

    # The manifest validator requires absolute paths and computes each file's checksum.
    input_manifest = Manifest(manifest_type=ManifestType.INPUT, files=[str(fmatch_cam_path.resolve())])
    manifest_path = input_manifest.write(output_dir)

    # The runner reads its dropbox from PROCESSING_PATH.
    os.environ["PROCESSING_PATH"] = str(output_dir)
    output_manifest_path = scene_id_cam.algorithm(manifest_path)

    # The output manifest registers the written SCENE-ID-CAM product(s); return the file it points at.
    output_manifest = Manifest.from_file(output_manifest_path)
    return Path(str(output_manifest.files[0].filename))
...
def main() -> None:
    ...
    parser.add_argument(
        "--date",
        type=lambda s: np.datetime64(datetime.strptime(s, "%Y-%m-%d"), "ns"),
        default=BASE_TIME,
        help=(
            "UTC date (YYYY-MM-DD) to anchor the synthetic footprint timeline, and hence the date embedded in every "
            f"generated filename. Default {str(BASE_TIME)[:10]}. Ignored for radiometer-timed modes when --l1b-file "
            "is given (those carry the L1B file's real timestamps)."
        ),
    )
    parser.add_argument(
        "--run-scene-id",
        action="store_true",
        help=(
            "After writing the FMATCH-CAM example, run the SCENE-ID-CAM algorithm on it and write the resulting "
            "SCENE-ID-CAM product into --output-dir. Because SCENE-ID-CAM inherits its RADIOMETER_TIME axis from the "
            "FMATCH-CAM input, both products share the same date. Runs entirely locally (no AWS)."
        ),
    )
    ...
    fmatch_cam_path: Path | None = None
    for mode in OperationalMode:
        path = generate_example_product(mode, args.output_dir, args.n_footprints, l1b_passthrough, args.date)
        ...
        # Remember the FMATCH-CAM file: it is the SCENE-ID-CAM runner's operational input.
        if mode is OperationalMode.CAM:
            fmatch_cam_path = path

    if args.run_scene_id:
        if fmatch_cam_path is None:  # defensive: OperationalMode.CAM is always generated above
            raise RuntimeError("FMATCH-CAM example was not generated; cannot run SCENE-ID-CAM")
        print(f"Running SCENE-ID-CAM on: {fmatch_cam_path.name}")
        scene_id_path = run_scene_id_cam(fmatch_cam_path, args.output_dir)
        print(f"  [           SCENE-ID-CAM] (from FMATCH-CAM ) -> {scene_id_path.name}")
